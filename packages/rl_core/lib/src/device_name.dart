...
/// Validates and normalises a justification string provided in a permission request.
///
/// Returns the trimmed, NFC-normalised string if valid, or `null` if rejected.
///
/// Rules:
...
/// - Strings containing zero-width or formatting marks (`\u200B`, `\uFEFF`, `\u2800`, `\u00AD`, etc.) are rejected.
/// - Leading and trailing whitespace is trimmed.
/// - Empty strings or strings that are empty after trimming return `null`.
/// - Strings containing only whitespace or zero-width / invisible characters return `null`.
/// - Strings are normalised to Unicode NFC.
/// - Strings exceeding [maxLength] user-perceived characters (grapheme clusters, default 256) after normalisation are rejected.
/// - Invalid justifications are rejected (returns `null`), never silently truncated or substituted.
String? sanitiseJustification(String? raw, {int maxLength = 256}) {
  if (raw == null) return null;
  ...
  // 6. Cap at maxLength user-perceived characters (grapheme clusters).
  if (normalised.characters.length > maxLength) {
    return null;
  }
